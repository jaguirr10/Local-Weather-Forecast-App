// http://api.apixu.com/v1/forecast.json?key=847bfb45ad1446baa9d224047171910&q=38.4127,27.1384&days=5

$(document).ready(function() {
    // var latitude,longitude;
    var api_key = "847bfb45ad1446baa9d224047171910&q=";
    var loc;
    ...
    $.getJSON('http://ipinfo.io', function(d) {
        ...
        $.getJSON(api, function(v1) {
           var city = v1.location.name;
            var state = v1.location.region;
            ...
            (function backgroundImageChange() {
                ...
                if (clear.indexOf(currentIconCode) !== -1 && nightTimes.indexOf(hours) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(clear.jpg)');
                } else if (clear.indexOf(currentIconCode) !== -1 && hours < 18 && hours > 6) {
                    $('#bg-image, #weather-box').css('background-image', 'url(sunny.jpg)');
                }

                // if weather partly cloudy
                if (partlyCloudy.indexOf(currentIconCode) !== -1 && nightTimes.indexOf(hours) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(partly-cloudy-night.jpg)');
                } else if (partlyCloudy.indexOf(currentIconCode) !== -1 && hours < 18 && hours > 6) {
                    $('#bg-image, #weather-box').css('background-image', 'url(partly-cloudy-day.jpg)');
                    console.log("hellooooo");
                }

                // if weather Cloudy
                if (cloudy.indexOf(currentIconCode) !== -1 && nightTimes.indexOf(hours) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(cloudy-night.jpg)');
                } else if (cloudy.indexOf(currentIconCode) !== -1 && hours < 18 && hours > 6) {
                    $('#bg-image, #weather-box').css('background-image', 'url(cloudy-day.jpg)');
                }


                // if weather lightRain
                if (lightRain.indexOf(currentIconCode) !== -1 && nightTimes.indexOf(hours) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(light-rain-night.jpg)');
                ...
                if (heavyRain.indexOf(currentIconCode) !== -1 && nightTimes.indexOf(hours) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(heavy-rain-night.jpg)');
                ...
                if (thunder.indexOf(currentIconCode) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(thunder.jpg)');
                }

                // if weather fog
                if (fog.indexOf(currentIconCode) !== -1 && nightTimes.indexOf(hours) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(fog-night.jpg)');
                } else if (fog.indexOf(currentIconCode) !== -1 && hours < 18 && hours > 6) {
                    $('#bg-image, #weather-box').css('background-image', 'url(fog-day.jpg)');
                }

                // if weather icePellets
                if (icePellets.indexOf(currentIconCode) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(ice-pellets.jpg)');
                }

                // if weather lightSnow
                if (lightSnow.indexOf(currentIconCode) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(snow-light.jpg)');
                }

                // if weather Snow
                if (snow.indexOf(currentIconCode) !== -1) {
                    $('#bg-image, #weather-box').css('background-image', 'url(snow.jpg)');
                }
            })();
            ...
            (function cityBox() {
                $("#city-box").html(city + ", " + state + ", " + country);
            ...
            (function buttonWind() {
                $('#btn-wind').click(function() {
                    /* Act on the event */
                    if (windSwap === false) {
                        $('#wind').hide().html(wind_kph).fadeIn(1800);
                        $('#btn-wind').html("<strong>kph</strong>");
                        windSwap = true;
                    } else {
                        $('#wind').hide().html(wind_mph).fadeIn(1800);
